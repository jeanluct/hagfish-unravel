function [t,L,R] = Lsolve_pinned_thread(m,smiley,L0,dLmax,unfrac)
...
if R < 0
  warning('Lsolve_pinned_thread:Rnegative','R=%g < 0 at t=%g.',R,t)
end

f = @(x) (x.^m + smiley*R*(x - 1));

% The thread equation to solve for x = dL/dt.
dL = fsolve(f,1,optimset('Display','off','TolX',1e-15));

if dL > 1
  warning('Lsolve_pinned_thread:dLabove1','dL/dt=%g > 1 at t=%g.',dL,t)
end

if dL < 0
  warning('Lsolve_pinned_thread:dLnegative','dL/dt=%g < 0 at t=%g.',dL,t)
end
